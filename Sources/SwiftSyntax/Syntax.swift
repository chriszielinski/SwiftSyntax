...
//  }
  ...
  /// The description parsed from the leading trivia to display to the user.
  /// Leading trivia is attached to the first token syntax contained by this
  /// node. Without such token, this property will return nil.
  public var message: String? {
    if let comment = leadingTrivia?.pieces.flatMap({ (piece) -> String? in
      guard case .lineComment(let text) = piece else { return nil }
      return text
    }).first {
      return comment.trimmingCharacters(in: CharacterSet(charactersIn: "/ "))
    }
...
public func ==(lhs: Syntax, rhs: Syntax) -> Bool {
  ...
  guard rhs.numberOfChildren != 0 else {
    return (lhs as? TokenSyntax)?.text == (rhs as? TokenSyntax)?.text
  }

  for index in 0..<lhs.numberOfChildren {
...
//public static func ==<T: Syntax>(lhs: T, rhs: Syntax) -> Bool {
//  guard let sameRHS = rhs as? T else {
//    return false
//  }
//  guard rhs == lhs else {
//    return false
...
/// Determines if two nodes point to the same memory.
public func ===(lhs: Syntax, rhs: Syntax) -> Bool {
